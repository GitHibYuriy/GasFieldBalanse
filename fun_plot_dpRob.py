...
def plot_dpRob(CD,values,lng_tuple):
    ...
    for i in range(1, len(CalcData)) :
        ...
        if CalcData.loc[i,'QgasSum']>=0:
            ...
            FactData.loc[i,'dpRob_C']=CalcData.loc[i,'dpRob_C']
    ...
    if nr > 0: 
        for j in range (0,nr):
            ...
            Rg['dpRob_C']=RegimeGroups[j]['dpRob_C']
            
            # Rg = Rg[np.logical_not(np.isnan(Rg.dpRob_C))]
            Rg = Rg.dropna(axis=0, how='any')
            y_maxRg = np.max(Rg['dpRob_C'])
            if y_maxRg > y_max:
                y_max = y_maxRg
            ...
            plt.step(Rg['DateCalc'], Rg['dpRob_C'],  
    ...
    try: 
        YF_ryad=np.array(FactData['dpRob_C'])
    ...
    plt.ylabel(lng_tuple['ls_dpRob'], fontsize=8)
    plt.xticks(fontsize=8)
    plt.yticks(fontsize=8)
    plt.title(lng_tuple['ls_dpRob_titul'], fontsize=8)
