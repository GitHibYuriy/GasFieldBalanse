...
def isChecked():
    ...
    chb1 = Checkbutton(frame, text="", variable=cbRes, onvalue=1, offvalue=0, command=isChecked)
    chb1.grid(row=0, column= 0, sticky=(E),pady = 1, padx =1)
    btnRes = Button(frame, text=textRes, state=tk.DISABLED, relief="flat", font=("Arial Bold", 10), width=18)
    btnRes.grid(row=0, column= 1, sticky=(W),pady = 1, padx =1)

    chb2 = Checkbutton(frame, text="", variable=cbPres, onvalue=1, offvalue=0, command=isChecked)
    chb2.grid(row=1, column= 0,sticky=(E),pady = 1, padx =1)
    btnPres = Button(frame, text=textPres, state=tk.DISABLED, relief="flat",font=("Arial Bold", 10), width=18)
    btnPres.grid(row=1, column= 1, sticky=(W),pady = 1, padx =1)

    chb3 = Checkbutton(frame, text="", variable=cbWpot, onvalue=1, offvalue=0, command=isChecked)
    chb3.grid(row=2, column=0, sticky=(E),pady = 1, padx =1)
    btnWpot = Button(frame, text=textWpot, state=tk.DISABLED, relief="flat",font=("Arial Bold", 10), width=18)
    btnWpot.grid(row=2, column= 1, sticky=(W),pady = 1, padx =1)

    chb4 = Checkbutton(frame, text="", variable=cbWind, onvalue=1, offvalue=0, command=isChecked)
    chb4.grid(row=3, column= 0, sticky=(E),pady = 1, padx =1)
    btnWind = Button(frame, text=textWind, state=tk.DISABLED, relief="flat",font=("Arial Bold", 10), width=18)
    btnWind.grid(row=3, column= 1, sticky=(W) ,pady = 1, padx =1)
    
    if cbRes.get() == 1:
        btnRes['state'] = tk.NORMAL
        btnRes.configure(text=lng_tuple['ls_opt'],fg = 'blue')
    else: 
        if cbRes.get() == 0:
            btnRes['state'] = tk.DISABLED
            btnRes.configure(text=lng_tuple['ls_n_opt'],fg = 'grey50' )
    if cbPres.get() == 1:
        btnPres['state'] = tk.NORMAL
        btnPres.configure(text=lng_tuple['ls_opt'],fg = 'blue')
    else: 
        if cbPres.get() == 0:
            btnPres['state'] = tk.DISABLED
            btnPres.configure(text=lng_tuple['ls_n_opt'],fg = 'grey50')
    if cbWpot.get() == 1:
        btnWpot['state'] = tk.NORMAL
        btnWpot.configure(text=lng_tuple['ls_opt'],fg = 'blue')
    else: 
        if cbWpot.get() == 0:
            btnWpot['state'] = tk.DISABLED
            btnWpot.configure(text=lng_tuple['ls_n_opt'],fg = 'grey50')    
    if cbWind.get() == 1:
        btnWind['state'] = tk.NORMAL
        btnWind.configure(text=lng_tuple['ls_opt'],fg = 'blue')
    else: 
        if cbWind.get() == 0:
            btnWind['state'] = tk.DISABLED
...
def draw_labels():
    ...
    lbl = Label( text=lng_tuple['ls_log_file'], height=1, anchor='e',
                bg='white', font=("Arial Bold", 10))    
    lbl.grid(row=8, column=0, sticky=(W+E),pady = 1, padx =1)
    lbl = Label( text=lng_tuple['ls_reserv_ini'], height=1, width=40, anchor='e',
                bg='white', font=("Arial Bold", 10))
    lbl.grid(row=9, column= 0,sticky=( W+E),pady = 1, padx =1)
    lbl = Label( text=lng_tuple['ls_p_ini'], height=1, width=40,
                anchor='e', bg='white', font=("Arial Bold", 10))
    lbl.grid(row=10, column= 0, sticky=( W+E),pady = 1, padx =1)
    lbl = Label( text=lng_tuple['ls_Water_Poten'], height=1,width=40,
                anchor='e', bg='white', font=("Arial Bold", 10))
    lbl.grid(row=11, column= 0, sticky=( W+E),pady = 1, padx =1)
    lbl = Label( text=lng_tuple['ls_Water_Index'], height=1, width=40,
               anchor='e', bg='white', 
                font=("Arial Bold", 10))
    lbl.grid(row=12, column= 0, sticky=(W+E),pady = 1, padx =1)
    lbl = Label( text=lng_tuple['ls_iter'], height=1,width=40,
               anchor='e', bg='white', 
                font=("Arial Bold", 10))
    lbl.grid(row=13, column= 0, sticky=(W+E),pady = 1, padx =1)
    lbl = Label( text=lng_tuple['ls_relax'], height=1, width=40,
                 anchor='e', bg='white', 
                font=("Arial Bold", 10))
    lbl.grid(row=14, column= 0, columnspan=(1),rowspan=(1),sticky=(W+E),pady = 1, padx =1)
...
def draw_buttons():
    ...
    Q_button = Button( root, text="Quit",  height=1, relief=tk.RAISED,
                              command =  lambda: os_exit,  bg = 'orchid', fg ='white' )
    Q_button.grid(row=28, column= 4, columnspan=(1),rowspan=(1),sticky=(E+W),pady = 2, padx =1)
...
root = tk.Tk()
root.title("Mатериальный баланс газового месторождения Version: " + '2.1.9 '+ '29.07.2022')
...
log_entry = Entry(root, textvariable=LogFilePath, font = ("Arial", 8 ))
log_entry.grid(row=8, column=2, columnspan=3 ,sticky=(W+E),pady = 1, padx =1)

Resv_entry = Entry(root,textvariable=values.loc[2], font=("Arial Bold", 8), justify = tk.RIGHT)
Resv_entry.grid(row=9, column= 2, sticky=(W+E),pady = 1, padx =1)

p_entry = Entry(root,textvariable=values.loc[3],font=("Arial Bold", 8), justify = tk.RIGHT)
p_entry.grid(row=10, column= 2, sticky=(W+E),pady = 1, padx =1)

Wpot_entry = Entry(root,textvariable=values.loc[7], font=("Arial Bold", 8),justify = tk.RIGHT)
Wpot_entry.grid(row=11, column= 2,sticky=(W+E),pady = 1, padx =1)

Wind_entry = Entry(root,textvariable=values.loc[8],font=("Arial Bold", 8), justify = tk.RIGHT)
Wind_entry.grid(row=12, column= 2, sticky=(W+E),pady = 1, padx =1)

Iter_entry = Entry(root,textvariable=values.loc[32], font=("Arial Bold", 8), justify = tk.RIGHT)
Iter_entry.grid(row=13, column= 2,sticky=(W+E),pady = 1, padx =1)

Relax_entry = Entry(root,textvariable=values.loc[33],  font=("Arial Bold", 8), justify =tk.RIGHT)
Relax_entry.grid(row=14, column= 2,sticky=(W+E),pady = 1, padx =1)
