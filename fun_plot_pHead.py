...
def plot_pHead(CD,values,lng_tuple):
    ...
    for i in range(1, len(CalcData)) :
        ...
        if CalcData.loc[i,'QgasSum']>=0:
            ...
            FactData.loc[i,'pHead_C']=CalcData.loc[i,'pHead_C']
    ...
    if nr > 0: 
        for j in range (0,nr):
            ...
            Rg['pHead_C']=RegimeGroups[j]['pHead_C']
            # Rg = Rg[np.logical_not(np.isnan(Rg.pHead_C))]
            Rg = Rg.dropna(axis=0, how='any')
            y_maxRg = np.max(Rg['pHead_C'])
            if y_maxRg > y_max:
                y_max = y_maxRg
            ...
            plt.step(Rg['DateCalc'], Rg['pHead_C'],  
    ...
    try: 
        YF_ryad=np.array(FactData['pHead_C'])
    ...
    plt.ylabel(lng_tuple['ls_pHead'], fontsize=8)
    plt.xticks(fontsize=8)
    plt.yticks(fontsize=8)
    plt.title(lng_tuple['ls_pHead_titul'], fontsize=8)
